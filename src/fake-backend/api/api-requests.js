module.exports = function (app) {
  ...
  app.get("/core/api/v1/requests", (req, res) => {
    ...
    const USER = require("./api-login").USER;
    ...
    const requests = DATA.requests.filter((request) => {
      ...
      if (searchText) {
        const employee = DATA.employees.find(
          (empl) => empl.id === request.employeeDetails.id
        );
        if (employee) {
          if (
            !employee.firstName
      ...
      const employeeId = request.employeeDetails.id;
      const employee = DATA.employees.find((emp) => emp.id === employeeId);
      if (role === "ROLE_TEAM_LEAD" && team.id !== employee.teamDetails.id) {
        return false;
      }
      return true;
    });
  ...
  app.patch(
    ...
    (req, res) => {
      ...
      if (request) {
        ...
        request.mdfUsr = USER.userDetails.employeeId;
